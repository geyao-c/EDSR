    ...
    def __init__(self, conv, scale, n_feats, bn=False, act=False, bias=True):
        ...
        if (scale & (scale - 1)) == 0:    # Is scale = 2^n?
            for _ in range(int(math.log(scale, 2))):
                m.append(conv(n_feats, 4 * 64, 3, bias))
...
class ECBUpsampler(nn.Sequential):
    def __init__(self, conv, scale, n_feats, bn=False, act=False, bias=True, with_idt=False, bias_type=True, num_conv_branches=1):
        ...
        if (scale & (scale - 1)) == 0:    # Is scale = 2^n?
            for _ in range(int(math.log(scale, 2))):
                ...
                m.append(ECB(n_feats, 4 * n_feats, depth_multiplier=2.0, act_type='linear', with_idt=with_idt,
                             bias_type=bias_type, num_conv_branches=num_conv_branches))
                m.append(nn.PixelShuffle(2))
                if bn:
                    m.append(nn.BatchNorm2d(n_feats))
                if act == 'relu':
