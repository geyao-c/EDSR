...
def make_model(args, parent=False):
    return PRelu_ECB_EDSR(args)

class PRelu_ECB_EDSR(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(PRelu_ECB_EDSR, self).__init__()
        ...
        pact = nn.PReLU(num_parameters=n_feats)
        # define body module
        m_body = [
            # common.ResBlock(
            ...
            common.ECBResBlock(
                conv, n_feats, kernel_size, act=pact, res_scale=args.res_scale
            )
            for _ in range(n_resblocks)
        ]
